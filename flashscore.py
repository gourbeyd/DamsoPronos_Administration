...
for page in pages:
    ...
    for elem in tab:
        liste_url_matchs.append(elem.get_attribute("id")[4:])#pour prendre en compte le _ ok
...
for k in range(len(liste_url_matchs)):
    ...
    print(f"HS {customHS} AS {customAS} HS {customHST} AS {customAST} league {league} gd {gameDate}")
    ...
    matchs.execute("""INSERT INTO MATCHS(id, date, HomeTeam, AwayTeam, ODD_HOME, ODD_DRAW, ODD_AWAY, OD_DRAW_OR_AWAY, PSHG, PSAG) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", (liste_url_matchs[k], gameDate, home_team, away_team, cotes[0], cotes[1], cotes[2], double_chance(cotes[1], cotes[2]), int(pshg), int(psag)))
